...
window.onload = () => {
    let equation = "V(r)=-\\frac{r_s}{r}+\\frac{L_z ^2}{m^2 c^2} \\left( \\frac{1}{r^2} - \\frac{r_s}{r^3} \\right)"; // Example equation
    katex.render(equation, document.getElementById('effective-potential'));

    equation = "\\scriptsize r_s = \\text{Schwarzschild radius of the sun} \\\\ \\scriptsize r = \\text{distance between the sun and Mercury} \\\\  \\scriptsize L_z = \\text{Mercury's angular momentum per unit mass}";
    katex.render(equation, document.getElementById('GR-key'));

    let code = `
function radialForce(position) {
    let newton = G * sunMass * mercury.mass / (position.mag()**2);
    let GRcorrection = 3 * G * (sunMass + mercury.mass) * L**2 / (c**2 * reducedMass * position.mag()**4);
    let total = newton + GRcorrection
    return total
    ...
    document.getElementById('highlight-code').textContent = code.trim();
    code = `
function rk4solver() {
    let denominator = (mercury.mass*r.mag());
    let k1 = r.copy().mult(radialForce(r) / denominator);
...
switch (integrator) {
    ...
    document.getElementById('RK4').textContent = code.trim();
    hljs.highlightAll();
}