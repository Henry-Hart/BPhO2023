...
function kepCartesianEllipseCenter(planet) {
    return {
        x: centerX + planet.a * planet.ecc * kepScaleMultiplier * Math.cos(planet.beta),
        y: centerY
    }
}
...
keplerCanvas.addEventListener('mousemove', function(event) {
    if (kepPlanetBeingDragged) {
        ...
        points = findIntersections(kepPlanetBeingDragged.a * kepScaleMultiplier, kepPlanetBeingDragged.ecc, kepPlanetBeingDragged.beta,
                                     (kepCartesianEllipseCenter(kepPlanetBeingDragged).x - centerX), 0, (mouseX - centerX), (centerY - mouseY))
...
window.onload = () => {
    equation = "(\\small a \\epsilon \\cos{\\beta}, 0)"
    katex.render(equation, document.getElementById('ellipse-center'))

    equation = "\\dfrac{\\left( \\dfrac{x}{\\cos{\\beta}}-a \\epsilon \\cos{\\beta} \\right)^2}{a^2} + \\dfrac{y^2}{a^2 (1-\\epsilon^2)}=1";
    katex.render(equation, document.getElementById('cartesian-ellipse'))

    equation = "m= \\dfrac{y_2-y_1}{x_2-x_1}"
    katex.render(equation, document.getElementById('m'))

    equation = "d = \\sqrt{(\\epsilon^2 - 1)\\left( a^2(m^2 \\cos^2\\beta + 1)(\\epsilon^2 -1)-2a\\epsilon m\\cos{\\beta}(mx_1-y_1)+(mx_1 - y_1)^2\\right)}"
    katex.render(equation, document.getElementById('d'))

    equation = "\\phi_1 = \\dfrac{(-a\\epsilon(\\epsilon^2 - 1) + +m\\cos\\beta (m x_1 - y_1) - d)\\cos\\beta}{-\\epsilon^2 + m^2 \\cos^2\\beta + 1} "
    katex.render(equation, document.getElementById('phi1'))

    equation = "\\phi_1 = \\dfrac{(-a\\epsilon(\\epsilon^2 - 1) + +m\\cos\\beta (m x_1 - y_1) + d)\\cos\\beta}{-\\epsilon^2 + m^2 \\cos^2\\beta + 1} "
    katex.render(equation, document.getElementById('phi2'))   

    equation = "\\lambda_1 = m(\\phi_1-x_1) + y_1"
    katex.render(equation, document.getElementById('lambda1'))

    equation = "\\lambda_2 = m(\\phi_2-x_1) + y_1"
    katex.render(equation, document.getElementById('lambda2'))

    equation = "\\text{return} ((\\phi_1, \\lambda_1), (\\phi_2, \\lambda_2))"
    katex.render(equation, document.getElementById('return'))
}