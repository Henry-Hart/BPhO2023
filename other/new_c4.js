...
get_challenge_4_generic_planets = (indxs) => {
    ...
    window.animate_current = () => {
        ...
        if(animate_running_toggle) {
            ...
            animate_3d(window.chart.series)
    ...
    window.reset_current = () => {
        ...
        //data = org_data[0]
        animate_3d(window.chart.series, true)
    }
...
initialise_challenge_4 = () => {
    ...
    // requires Highcharts to be loaded
try {
    window.org_colors = Highcharts.getOptions().colors
...
get_challenge_4_closer_data = () => {
    ...
    if(loc[0] == 4) {
        get_challenge_4_closer_planets()
        try {
            if(window["reset_current"]) reset_current()
        } catch {}
        challenge_4_resize()
        ...
        document.getElementById("animate_overlay").style.display = "inherit"
        document.getElementById("animate_break").style.display = "inherit"
    }
...
challenge_4_resize = () => {
    try {
    if(loc[0] === 4) {
        x = preloading.offsetWidth / container.offsetWidth;
        y = preloading.offsetHeight / container.offsetHeight;
        a = x < y ? x : y ;
        document.getElementById("container").style.transform = "translate(-50%, -50%) scale("+a+")"
    }
    } catch {}
}

window.addEventListener('resize', challenge_4_resize);
...
//animate_speed = 0.1
// global hell.
function animate_3d(data, onetime=false) {
    ...
    function __internal__animate() {
        ...
        if(!onetime) requestAnimationFrame(__internal__animate)
    };
...
challenge_4_setup_generic = (axes, _data) => {
...
get_challenge_4_closer_planets = () => {
    _data = get_challenge_4_generic_planets([9,8,10,7])//[1,2,3,5,6])
    axes = [[-1.7, 1.7], [-1.7, 1.7], [-1.7, 1.7]]
    challenge_4_setup_generic(axes, _data)
    return null
}

get_challenge_4_further_planets = () => {
    _data = get_challenge_4_generic_planets([1,2,3,5,6])
    axes = [[undefined, undefined], [-50, 50], [undefined, undefined]]
    challenge_4_setup_generic(axes, _data)
    return null
}